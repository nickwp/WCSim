...
class WCSimWCHit : public G4VHit
{
 ...
 public:
  ...
  void AddPhotonHistory (const std::vector<G4int> &photHistory) { photonHistory.push_back(photHistory); }
  ...
  std::vector<G4int> GetPhotonHistory(int i) { return photonHistory[i]; };
 ...
 private:
  ...
  G4int                 totalPeInGate;
  std::vector<std::vector<G4int>> photonHistory;
};
